          ...
          <td>
            <div class="ac_di">
                <ul>
                    ...
                    <li>
                      <span title='Delete manager' style="cursor: pointer;" onclick="delete_manager({{$item->id}})" data-toggle="modal" data-target="#user_delete_modal" class="badge badge-light  badge-lg"><img src="{{ url('public') }}/assets/images/action3.png" alt=""></span>
                    </li>
                    @else
                    ...
                    @endif
                    <li>
                      <span style="cursor: pointer;" onclick="fetch_user_details({{$item->id}})" data-toggle="modal" data-target="#user_details_modal" class="badge badge-light  badge-lg"><img src="{{ url('public') }}/assets/images/about.png" alt=""></span>
                      ...
                      <input type="hidden" id="worker_identity_{{$item->id}}" value="{{url('public/uploads/'.$item->identity_doc) }}">
               ...
               <div class="col-md-12 text-center mb-5">
                ...
                <div class="col-md-12">
                  <div class="user_data_box">
                    ...
                    <ul class="mb-2">
                      <li class="ttle">Identity  : </li>  
                      <li class="details">
                          <div class='identity_div'></div>
                      </li>  
                    </ul>
                  </div>
                </div>
...
<div class="modal fade" id="dust_return_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
  ...
  <div class="modal fade" id="worker_forward_modal">
    ...
    <div class="modal fade" id="user_delete_modal">
        <div class="modal-dialog">
            <div class="modal-content">
                <!-- Modal Header -->
                <div class="modal-header">
                    <h4 class="modal-title">Forward all workers of this manager</h4>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
                <div class="modal-body">
                    
                    <form action="{{url('delete_manager')}}" method='post'>
                        @csrf
                        <input type="hidden" id="manager_id" name="manager_id">
                         <label for="exampleSelect">Select Manager</label>
                            <select class="text-uppercase form-control mb-3" id="delete_manager_bo" required name="new_managet_id">
                                @foreach ($worker_forward as $item){
                                    <option value='{{$item->id}}'>{{$item->name}}</option>
